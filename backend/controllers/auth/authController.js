...
const schoolShape = (school) => ({
  _id:     school._id,
  name:    school.name,
  slug:    school.domain || school.slug,
  domain:  school.domain || school.slug,
...
const userShape = (user) => {
  // Check if childId is a populated object (from Mongoose) or just a string
  ...
  return {
    ...
    // For Parents: Extract details from the populated childId object
...
exports.parentSendOtp = async (req, res) => {
  try {
    ...
    console.log ("Generated OTP for +977-" + phone + ": " + otpPlain); // Log OTP for debugging (remove in production)
