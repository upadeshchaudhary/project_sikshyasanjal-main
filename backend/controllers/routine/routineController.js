...
function getSubjectFrequency(className) {
  ...
  if (classNum >= 1 && classNum <= 5) {
    ...
    const coreSubjects = ["English","Nepali","Science","Social Studies","Maths","Computer","Health"];
    coreSubjects.forEach(s => { freq[s] = 5; });
    return freq;
  }
...
for (let i = 0; i < 5; i++) {
  ...
  pool.forEach(subject => {
    ...
    if (available.length === 0) {
      throw new Error(`Failed to place subject "${subject}" without duplication. Check subject frequency settings.`);
    }
...
exports.getRoutineByClass = async (req, res) => {
  try {
   ...
   //Strip Saturday/Sunday from response even if old data had them
    const clean = { ...routine };
