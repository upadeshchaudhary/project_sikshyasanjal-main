...
// ════════════════════════════════════════════════════════════════════════════════
// POST /api/teachers/:id/force-logout — Invalidate all sessions for a teacher
// Admin only
// Implementation: updates passwordHash to a random value, forcing all existing tokens to become invalid
// ════════════════════════════════════════════════════════════════════════════════
exports.forceLogoutTeacher = async (req, res) => {
  try {
    ...
    // Invalidate all existing tokens by updating passwordHash with a random string
    await User.findByIdAndUpdate(req.params.id, {
      $set: { passwordHash: await bcrypt.hash(Math.random().toString(36), 12) },
    });

    res.json({
      success: true,
      message: `All sessions for teacher "${teacher.name}" have been invalidated. They will be forced to log in again.`,
    });
  } catch (err) {
