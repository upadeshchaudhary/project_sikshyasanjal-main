...
export default function AdminLogin() {
  ...
  const handleSubmit = async () => {
    ...
    if (!isValidEmail(userData.email)) errs.email    = "Enter a valid email address";
    ...
    try {
      ...
      if (res.status === 200) {
        // ✅ FIX: correct arg order — login(token, user, schoolData)
        // AppContext.login handles localStorage + axios headers internally
    ...
    <div style={s.page}>
      ...
      {offline && (
        <div style={s.offlineBanner}>
          Backend unavailable. Login and API actions require the backend.
        </div>
      )}

      <div style={s.card}>
        <FormStep
          email={userData.email}       setEmail={(v) => setUserData({ ...userData, email: v })}
          password={userData.password} setPassword={(v) => setUserData({ ...userData, password: v })}
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
        ...
        <div>
          <div style={{ fontWeight: 700, fontSize: 14, color: "#1A1A2E" }}>School Administrator</div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        <ActionButton loading={loading} onClick={onSubmit} color="#1E3FF2">
          Sign In
        </ActionButton>
      </div>
    </div>
...
function ActionButton({ color = "#1E3FF2", loading, onClick, children, disabled }) {
  return (
    <button
      onClick={onClick}
